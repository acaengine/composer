...
* @Last Modified time: 2017-03-24 15:06:44
*/
...
export let initialiseMockClasses = () => {
	...
	for(let i = 0; i < mod_cnt; i++){
		...
		MOCK_REQ_HANDLER.register(`/control/api/module/${module.id}`, false, module);
	}
	MOCK_REQ_HANDLER.register('/control/api/modules', true, MODULE_LIST);
		// Add mock zones to the system
	let zone_cnt = Math.floor(Math.random() * 20 + 2);
	for(let i = 0; i < zone_cnt; i++){
		let zone = new MockZone();
		ZONE_LIST.push(zone);
		MOCK_REQ_HANDLER.register(`/control/api/zone/${zone.id}`, false, zone);
	}
	MOCK_REQ_HANDLER.register('/control/api/zones', true, ZONE_LIST);
	...
	if(self && self['control'] && self['control']['systems']) {
		...
		for(let i = 0; i < sys_cnt; i++){
			...
			MOCK_REQ_HANDLER.register(`/control/api/system/${system.id}`, false, system);
		}
	}
	MOCK_REQ_HANDLER.register('/control/api/systems', true, SYSTEM_LIST);
		// Add mock user to the system
	MOCK_REQ_HANDLER.register('/control/api/user/current', false, new MockUser());
}
