...
export class BindingDirective implements OnChanges, OnDestroy, OnInit {
    ...
    public ngOnChanges(changes: any) {
        ...
        if (this.init) {
            ...
            if (changes.exec) {
                log('BIND(D)', `${this.id}: Execute function changed. ${changes.exec.previous || ''} → ${this.exec}`);
                ...
                log('BIND(D)', `${this.id}: Local value changed. ${old_value} → ${this.value}`);
                this.call()
            }
        } else if (!this.init) {
    ...
    public call_exec(exec?: string) {
        ...
        log('BIND(D)', `${this.id}: Executing function '${exec}' from directive: ${bind_info}`);
