...
export class EngineStatusVariable {
    ...
    public setValue(value: any, local: boolean = false) {
        ...
        if (!local) {
            ...
            log('STATUS', `Local value changed calling exec(${this.binding}). ${previous} → ${value}`);
            this.exec();
        }
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            if (this.value('bindings') <= 0) {
                ...
                (this.service as any).io.bind(system.id, module.id, module.index, this.id).then(() => {
                    log('STATUS', `Bound to '${this.id}' on ${this.parent.parent.id}, ${mod}`, this.value());
                    resolve(() => { this.unbind() });
                }, (err) => {
                    error('STATUS', 'Binding to status variable failed.', err);
                    reject(err);
                });
            }
    ...
    public rebind() {
        if (this.value('bindings') > 0) {
            const module = this.parent;
            const system = module.parent;
            log('STATUS', `Rebinding to ${this.id} on ${system.id}, ${module.id} ${module.index}`);
            (this.service as any).io.bind(system.id, module.id, module.index, this.id).then(() => {
                log('STATUS', `Rebound to ${this.id} on ${system.id}, ${module.id} ${module.index}`, this.value());
            }, (err) => error('STATUS', 'Binding to status variable failed.', err));
        }
    }
    ...
    public unbind() {
        ...
        if (count === 1) {
            ...
            (this.service as any).io.unbind(system.id, module.id, module.index, this.id).then(() => {
                ...
                log('STATUS', `Unbound binding(${this.binding}). 0 remaining.`);
            }, (err) => {
                error('VAR', 'Unbinding from status variable failed.', err);
                this.subjects.bindings.next((this.value('bindings') || 0) + 1);
            });
        } else if (count > 1) {
            this.subjects.bindings.next(count - 1);
            log('STATUS', `Unbound binding(${this.binding}). ${count - 1} remaining.`);
        }
    }
