...
export class CommsService {
     ...
     */
    public tryLogin() {
        log('COMMS', `Trying Login`);
        this.login().then(() => log('COMMS', `Got Access Token.`), (e) => null);
    }
                    ...
                    .subscribe(
                        ...
                        (err: any) => {
                            ...
                            if (err && err.status === 401
                                ...
                                log('COMMS', `Failed token refresh request for ${url}`);
                                ...
                                error('COMMS', `Refresh failed with code 0. Headers may be malformed or missing CORS.`);
                                this.processLoginError(err).then((i) => resolve(i), (e) => reject(e));
                            } else {
                                this.processLoginError(err).then((i) => resolve(i), (e) => reject(e));
                            }
                        }, () => {
                            log('COMMS', `Got new tokens:`, tokens);
            ...
            oauth.hasValidAccessToken().then((valid: boolean) => {
                if (valid) {
                    ...
                    oauth.tryLogin().then(() => {
                        ...
                        log('COMMS', `Log in attempt failed...`, null, 'warn');
        ...
        return new Promise((resolve, reject) => {
            ...
            this.store.getItem(`${oauth.get('client_id')}_refresh_token`).then((refresh: string) => {
                if (refresh || oauth.model.code) { // Refresh token exists
                    log('COMMS', `Refresh token found. Refreshing access token...`);
                    // Perform refresh
                    ...
                    log('COMMS', `No Refresh Token or Code`);
                    let path = location.href;
                    if (location.hash.indexOf(path) >= 0
                        && location.href.indexOf(location.origin + '/#/') >= 0) {
    ...
    private error(err: any, req: any, obs: Subscriber<{}>) {
        ...
        error('COMMS', `Request to ${req.url} failed with code ${err ? err.status : 0}. ${err ? err.message : ''}`);
        ...
        if (err && err.status === 401 && this.retry[hash] < 3 && !mock) {
            ...
            log('COMMS', `Re-authenticating...`);
            this.retryAfterAuth(err, req, obs, hash);
        } else if (err && err.status === 401 && !mock && err.clear !== false) {
            error('COMMS', `Error with auth details restarting fresh.`);
            this.oAuthService.reset();
            obs.error(err);
            this.retry[hash] = 0;
        } else { // Return error
            log('COMMS', `Error processing request(${err.status}).`, err);
    ...
    private retryAfterAuth(err, req, obs: Subscriber<{}>, hash: string) {
        this.login().then(() => {
            log('COMMS', `Retrying request to '${req.url}'...`);
            ...
            error('COMMS', `Error logging in.`, retry_err);
