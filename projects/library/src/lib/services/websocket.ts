...
export class WebSocketInterface {
    ...
    private onmessage(evt: any) {
        ...
        if (msg.type === SUCCESS || msg.type === ERROR || msg.type === NOTIFY) {
            ...
            if (msg.type === ERROR) {
                error('WS', `Received error(${msg.id}). ${msg.msg}`);
            } else if (msg.type === NOTIFY) {
                log(`WS`, `Received notify. ${meta_list} →`, msg.value);
            } else {
                if (meta) {
                    log(`WS`, `Received success(${msg.id}). ${meta_list}`);
                } else {
                    log(`WS`, `Received success(${msg.id}). Value: ${msg.value}`);
                }
            }
            if (msg.type === SUCCESS) {
    ...
    private fail(msg: any, type: any) {
        error('WS', `Failed ${type}. ${JSON.stringify(msg)}`);
        return false;
    }
        ...
        return new Promise<any>((resolve) => {
            if (!this.io || this.io.readyState !== this.io.OPEN) {
                ...
                this.connect().then(() => {
                    ...
                    log('WS', `Failed to connect(${type}, ${name}). ${error}`);
            ...
            log('WS', `Sent ${type} request(${request.id}). ${system}, ${mod}, ${index}, ${name}`, args);
            if (args !== null) {
                request.args = args;
            }
